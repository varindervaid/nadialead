...
class leadRatingService
{
    public function getList($inputs)
    {
        ...
        try {
            ...
            // Initialize query
            $leadTags = LeadRating::orderBy($sortBy, $sortOrder);
            ...
            Log::error('Lead rating fetch service failed', ['error' => $e->getMessage()]);
            throw $e;
        }
    ...
    public function store($inputs)
    {
        ...
        try {
            $model = LeadRating::create($inputs);
            return $model;
        } catch (\Exception  | RequestException $e) {
            Log::error('Lead rating store service', ['error' => $e->getMessage()]);
            throw $e;
        }
    }

    public function show($leadRating)
    {
        return $leadRating;
    }
    ...
    {
        try {
            $leadTag = LeadRating::find($id);

            $leadTag->update($inputs);
            return $leadTag;
        } catch (\Exception  | RequestException $e) {
            Log::error('Lead rating update service', ['error' => $e->getMessage()]);
            throw $e;
        }
    }
    ...
    public function destroy($id){
        try {
            $leadTag = LeadRating::findOrFail($id);  // Find the record or throw a 404 exception

            $leadTag->delete();  // Delete the record

            return true;  // Return true on successful deletion
        } catch (\Exception | RequestException $e) {
            Log::error('Lead rating delete service', ['error' => $e->getMessage()]);
